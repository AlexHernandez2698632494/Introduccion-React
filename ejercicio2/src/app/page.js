...
export default function Home() {
  ...
  const potencia = () => {
    const base = parseFloat(numero1);
    const exponente = parseFloat(numero2);
    const resultadoPotencia = Math.pow(base, exponente);
    setResultado(`Resultado de la potencia: ${resultadoPotencia}`);
  };

  const calcularRaiz = () => {
    const indice = parseFloat(numero1);
    const numero = parseFloat(numero2);
    const resultadoRaiz = Math.pow(numero, 1 / indice);
    setResultado(`Resultado de la raíz ${indice}: ${resultadoRaiz}`);
  };
        ...
        <div className={styles.calculadora}>
          ...
          <div className={styles.buttonGroup}>
            ...
            <div>
              ...
              <button className={styles.button} onClick={calcularRaiz}>Raíz</button>
            </div>
          </div>
